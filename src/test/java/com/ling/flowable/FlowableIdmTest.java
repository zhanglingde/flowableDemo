...
class FlowableIdmTest {
    ...
    private static final Logger logger = LoggerFactory.getLogger(FlowableIdmTest.class);
    ...
    @Test
    void test07() {
        List<User> list = identityService.createNativeUserQuery().sql("select * from ACT_ID_USER where EMAIL_=#{email}").parameter("email", "zhangling@qq.com").list();
        for (User user : list) {
    ...
    @Test
    void test05() {
        List<User> list = identityService.createUserQuery().userDisplayNameLike("%张%").list();
        for (User user : list) {
            logger.info("id:{};displayName:{}", user.getId(), user.getDisplayName());
        }
    }
    ...
    @Test
    void test04() {
        identityService.deleteUser("ling");
    }

    @Test
    void test03() {
        User user = identityService.createUserQuery().userId("ling").singleResult();
    ...
    @Test
    void contextLoads() {
        //创建一个用户对象  act_id_user
        UserEntityImpl user = new UserEntityImpl();
        user.setId("ling");
        user.setDisplayName("张灵");
        user.setPassword("123");
        user.setFirstName("zhang");
        user.setLastName("ling");
        user.setEmail("zhangling@qq.com");
